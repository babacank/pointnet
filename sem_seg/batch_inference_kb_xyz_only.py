...
from model_kb_xyz_only import *
import indoor3d_util_kb_xyz_only
...
def evaluate():
    ...
    # with tf.device('/gpu:' + str(GPU_INDEX)):
    #     pointclouds_pl = tf.placeholder(tf.float32, shape=(1,4096,3))
    #     labels_pl = placeholder_inputs(BATCH_SIZE, NUM_POINT)
    #     is_training_pl = tf.placeholder(tf.bool, shape=())
    ...
    saver = tf.train.import_meta_graph(os.path.join(BASE_DIR, "log/model_kb.ckpt.meta"))
...
def eval_one_epoch(sess, ops, room_path, out_data_label_filename, out_gt_label_filename):
    ...
    current_data, current_label = indoor3d_util_kb_xyz_only.room2blocks_wrapper_kb(room_path, NUM_POINT)
    ...
    for batch_idx in range(num_batches):
        ...
        for b in range(BATCH_SIZE):
            ...
            for i in range(NUM_POINT):
                color = indoor3d_util_kb_xyz_only.g_label2color[pred[i]]
                color_gt = indoor3d_util_kb_xyz_only.g_label2color[current_label[start_idx+b, i]]
                if FLAGS.visu:
                    fout.write('v %f %f %f %d %d %d\n' % (pts[i,0], pts[i,1], pts[i,2], color[0], color[1], color[2]))
                    fout_gt.write('v %f %f %f %d %d %d\n' % (pts[i,0], pts[i,1], pts[i,2], color_gt[0], color_gt[1], color_gt[2]))
                fout_data_label.write('%f %f %f %f %d\n' % (pts[i,0], pts[i,1], pts[i,2], pred_val[b,i,pred[i]], pred[i]))
