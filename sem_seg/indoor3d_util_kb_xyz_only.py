...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    if not random_sample:
        num_block_x = abs(int(np.ceil((limit[0] - block_size) / stride))) + 1
        num_block_y = abs(int(np.ceil((limit[1] - block_size) / stride))) + 1
    ...
    for idx in range(len(xbeg_list)):
        xbeg = xbeg_list[idx]
        ybeg = ybeg_list[idx]
        xcond = (abs(data[:, 0]) <= xbeg + block_size) & (abs(data[:, 0]) >= xbeg)
        ycond = (abs(data[:, 1]) <= ybeg + block_size) & (abs(data[:, 1]) >= ybeg)
        cond = xcond & ycond
...
def room2blocks_plus_kb(data_label, num_point, block_size, stride,
                     random_sample, sample_num, sample_aug):
    """ room2block with input filename and RGB preprocessing.
    """
    data = data_label[:, 0:3]
...
def room2blocks_wrapper_kb(data_label_filename, num_point, block_size=0.5, stride=0.25,
                           random_sample=False, sample_num=None, sample_aug=1):
    if data_label_filename[-3:] == 'txt':
        data_label = np.loadtxt(data_label_filename)
    elif data_label_filename[-3:] == 'npy':
        ...
        print('Unknown file type! exiting.')
        exit()
    return room2blocks_plus_kb(data_label, num_point, block_size, stride,
                               random_sample, sample_num, sample_aug)
...
def room2blocks_plus_normalized_kb(data_label, num_point, block_size, stride,
                                   random_sample, sample_num, sample_aug):
    """ room2block, with input filename and RGB preprocessing.
        for each block centralize XYZ, add normalized XYZ as 678 channels
    """
...
def room2blocks_wrapper_normalized_kb(data_label_filename, num_point, block_size=1.0, stride=1.0,
                                      random_sample=False, sample_num=None, sample_aug=1):
    if data_label_filename[-3:] == 'txt':
        data_label = np.loadtxt(data_label_filename)
    elif data_label_filename[-3:] == 'npy':
        ...
        print('Unknown file type! exiting.')
        exit()
    return room2blocks_plus_normalized_kb(data_label, num_point, block_size, stride,
                                          random_sample, sample_num, sample_aug)
